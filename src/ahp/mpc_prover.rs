...
pub struct ProverState<'a, 'b, F: PrimeField, C> {
    formatted_random_assignment: Vec<F>,
...
impl<F: Field> ProverMsg<F> {
    /// Create a new instance of empty message
    pub fn new() -> Self {
        Self {
            field_elements: vec![],
        }
...
/// The statement oracles of the prover.
/// We don't need a proof for these
pub struct ProverStatmentOracles<'b, F: Field> {
    /// The LDE of `x`.
    pub x: LabeledPolynomial<'b, F>,
    /// The LDE of `r`
    pub r: LabeledPolynomial<'b, F>,
}

impl<'b, F: Field> ProverStatmentOracles<'b, F> {
    /// Iterate over the statment polynomials output by the prover in the first round.
...
impl<F: PrimeField> AHPForR1CS<F> {
    ...
    pub fn prover_init<'a, 'b, C: ConstraintSynthesizer<F>>(
        ...
        let formatted_random_assignment =
            ProverConstraintSystem::format_random_shares_input(rand_assignments);
    ...
    > {
        ...
        // let v_H : ff_fft::polynomial::dense::DensePolynomial<F> = domain_h.vanishing_polynomial().into();

        let x_time = start_timer!(|| "Computing x polynomial and evals");
        let domain_x = EvaluationDomain::new(formatted_input_assignment.len())
            .ok_or(SynthesisError::PolynomialDegreeTooLarge)?;
        let x_poly =
            EvaluationsOnDomain::from_vec_and_domain(formatted_input_assignment.clone(), domain_x)
                .interpolate();
        let rand_share_poly =
            Polynomial::from_coefficients_vec(formatted_random_assignment.clone());
        assert_eq!(rand_share_poly.degree(), 1);
        ...
        let z_a_poly =
            EvaluationsOnDomain::from_vec_and_domain(z_a.unwrap(), domain_h).interpolate();
        ...
        let z_b_poly =
            EvaluationsOnDomain::from_vec_and_domain(z_b.unwrap(), domain_h).interpolate();
        // + &(&Polynomial::from_coefficients_slice(&[F::rand(rng)]) * &v_H);
        ...
        // assert!(x_poly.degree() <= 3 * domain_h.size() + 2 * zk_bound - 3);
        ...
        // Additionally create a commitment to the x-polynomial with zk-bound of 1
        let x = LabeledPolynomial::new_owned("x".to_string(), x_poly, None, Some(zk_bound));
        let r = LabeledPolynomial::new_owned("r".to_string(), rand_share_poly, None, None);
        let mask_poly =
        ...
        let stmt_oracles = ProverStatmentOracles { x: x, r: r };
        ...
        Ok((msg, oracles, stmt_oracles, state))
    }
    ...
    pub fn prover_second_round<'a, 'b, R: RngCore, C: ConstraintSynthesizer<F>>(
        ...
        for k in 0..prover_state.domain_k.size() {
            ...
            *r_a_alpha_vals_on_H
                .entry(a_col_at_kappa)
                .or_insert(F::zero()) += &to_add_a;
            ...
            *r_b_alpha_vals_on_H
                .entry(b_col_at_kappa)
                .or_insert(F::zero()) += &to_add_b;
            ...
            *r_c_alpha_vals_on_H
                .entry(c_col_at_kappa)
                .or_insert(F::zero()) += &to_add_c;
        }
        ...
        let summed_r_m =
            EvaluationsOnDomain::from_vec_and_domain(summed_r_m_evals, domain_h).interpolate();
    ...
    pub fn prover_third_round<'a, 'b, R: RngCore, C: ConstraintSynthesizer<F>>(
        ...
        let VerifierSecondMsg { beta_1 } = *ver_message;

        let r_alpha_poly = r_alpha_poly
            .expect("ProverState should include r_alpha_poly when prover_third_round is called");
        ...
        let r_x_x_precomp: BTreeMap<_, _> = domain_h
            ...
            .zip(domain_h.batch_eval_unnormalized_bivariate_lagrange_poly_with_same_inputs())
            .collect();
        let r_beta_1_x_precomp: BTreeMap<_, _> = domain_h
            .elements()
            .zip(domain_h.batch_eval_unnormalized_bivariate_lagrange_poly_with_diff_inputs(beta_1))
            .collect();
        ...
        for k in 0..prover_state.domain_k.size() {
            ...
            let a_col_at_kappa = matrix_evals_on_K.a_col_evals[k];
            let a_row_at_kappa = matrix_evals_on_K.a_row_evals[k];
            let to_add_a = r_x_x_precomp[&a_row_at_kappa]
                * &r_beta_1_x_precomp[&a_col_at_kappa]
                * &matrix_evals_on_K.a_val_evals[k];
            *a_beta_1_vals_on_H
                .entry(a_row_at_kappa)
                .or_insert(F::zero()) += &to_add_a;

            let b_col_at_kappa = matrix_evals_on_K.b_col_evals[k];
            let b_row_at_kappa = matrix_evals_on_K.b_row_evals[k];
            let to_add_b = r_x_x_precomp[&b_row_at_kappa]
                * &r_beta_1_x_precomp[&b_col_at_kappa]
                * &matrix_evals_on_K.b_val_evals[k];
            *b_beta_1_vals_on_H
                .entry(b_row_at_kappa)
                .or_insert(F::zero()) += &to_add_b;

            let c_col_at_kappa = matrix_evals_on_K.c_col_evals[k];
            let c_row_at_kappa = matrix_evals_on_K.c_row_evals[k];
            let to_add_c = r_x_x_precomp[&c_row_at_kappa]
                * &r_beta_1_x_precomp[&c_col_at_kappa]
                * &matrix_evals_on_K.c_val_evals[k];
            *c_beta_1_vals_on_H
                .entry(c_row_at_kappa)
                .or_insert(F::zero()) += &to_add_c;
        }
        end_timer!(m_beta_1_evals_time);

        let summed_m_beta_1_time = start_timer!(|| "Computing summed_m_beta_1 poly");
        let summed_m_beta_1_evals = domain_h
            .elements()
            .map(|h_elem| {
        ...
        let summed_m_beta_1 =
            EvaluationsOnDomain::from_vec_and_domain(summed_m_beta_1_evals, domain_h).interpolate();
    ...
    pub fn prover_fourth_round<'a, 'b, R: RngCore, C: ConstraintSynthesizer<F>>(
        ...
        let VerifierSecondMsg { beta_1 } = verifier_second_msg.expect(
            "ProverState should include verifier_second_msg when prover_fourth_round is called",
        );
        ...
        for idx in 0..domain_k.size() {
            inverses_a.push(
                (beta_2 - &matrix_evals_on_K.a_row_evals[idx])
                    * &(beta_1 - &matrix_evals_on_K.a_col_evals[idx]),
            );
            inverses_b.push(
                (beta_2 - &matrix_evals_on_K.b_row_evals[idx])
                    * &(beta_1 - &matrix_evals_on_K.b_col_evals[idx]),
            );
            inverses_c.push(
                (beta_2 - &matrix_evals_on_K.c_row_evals[idx])
                    * &(beta_1 - &matrix_evals_on_K.c_col_evals[idx]),
            );
        }
        ...
        for idx in 0..domain_k.size() {
            let f_3_at_kappa = v_H_at_beta_2
                * &v_H_at_beta_1
        ...
        let domain_b = EvaluationDomain::<F>::new(6 * domain_k.size() - 6)
            .ok_or(SynthesisError::PolynomialDegreeTooLarge)?;

        let beta_minus_matrix_eval_time =
            start_timer!(|| "Computing beta_{1,2} - M_{row,col,val} evals on B");

        let beta_2_minus_a_row_on_B = cfg_iter!(matrix_evals_on_B.a_row_evals.evals)
            .map(|a| beta_2 - a)
            .collect();
        let beta_2_minus_a_row_on_B =
            EvaluationsOnDomain::from_vec_and_domain(beta_2_minus_a_row_on_B, domain_b);

        let beta_1_minus_a_col_on_B = cfg_iter!(matrix_evals_on_B.a_col_evals.evals)
            .map(|a| beta_1 - a)
            .collect();
        let beta_1_minus_a_col_on_B =
            EvaluationsOnDomain::from_vec_and_domain(beta_1_minus_a_col_on_B, domain_b);

        let beta_2_minus_b_row_on_B = cfg_iter!(matrix_evals_on_B.b_row_evals.evals)
            .map(|b| beta_2 - b)
            .collect();
        let beta_2_minus_b_row_on_B =
            EvaluationsOnDomain::from_vec_and_domain(beta_2_minus_b_row_on_B, domain_b);

        let beta_1_minus_b_col_on_B = cfg_iter!(matrix_evals_on_B.b_col_evals.evals)
            .map(|b| beta_1 - b)
            .collect();
        let beta_1_minus_b_col_on_B =
            EvaluationsOnDomain::from_vec_and_domain(beta_1_minus_b_col_on_B, domain_b);

        let beta_2_minus_c_row_on_B = cfg_iter!(matrix_evals_on_B.c_row_evals.evals)
            .map(|c| beta_2 - c)
            .collect();
        let beta_2_minus_c_row_on_B =
            EvaluationsOnDomain::from_vec_and_domain(beta_2_minus_c_row_on_B, domain_b);

        let beta_1_minus_c_col_on_B = cfg_iter!(matrix_evals_on_B.c_col_evals.evals)
            .map(|c| beta_1 - c)
            .collect();
        let beta_1_minus_c_col_on_B =
            EvaluationsOnDomain::from_vec_and_domain(beta_1_minus_c_col_on_B, domain_b);
        ...
        let a_poly_on_B = cfg_into_iter!(0..domain_b.size())
            .map(|idx| {
                v_H_at_beta_2
                    * &v_H_at_beta_1
                    * &((eta_a
                        * &matrix_evals_on_B.a_val_evals.evals[idx]
                        * &beta_2_minus_b_row_on_B.evals[idx]
                        * &beta_1_minus_b_col_on_B.evals[idx]
                        * &beta_2_minus_c_row_on_B.evals[idx]
                        * &beta_1_minus_c_col_on_B.evals[idx])
                        + &(eta_b
                            * &matrix_evals_on_B.b_val_evals.evals[idx]
                            * &beta_2_minus_a_row_on_B.evals[idx]
                            * &beta_1_minus_a_col_on_B.evals[idx]
                            * &beta_2_minus_c_row_on_B.evals[idx]
                            * &beta_1_minus_c_col_on_B.evals[idx])
                        + &(eta_c
                            * &matrix_evals_on_B.c_val_evals.evals[idx]
                            * &beta_2_minus_a_row_on_B.evals[idx]
                            * &beta_1_minus_a_col_on_B.evals[idx]
                            * &beta_2_minus_b_row_on_B.evals[idx]
                            * &beta_1_minus_b_col_on_B.evals[idx]))

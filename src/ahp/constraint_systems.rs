...
impl<ConstraintF: Field> ConstraintSystem<ConstraintF> for IndexerConstraintSystem<ConstraintF> {
    ...
    fn enforce<A, AR, LA, LB, LC>(&mut self, _name: A, a: LA, b: LB, c: LC)
    where
        A: FnOnce() -> AR,
        AR: Into<String>,
...
pub(crate) fn matrix_to_polys<F: PrimeField>(
    ...
    let row_evals_on_B =
        EvaluationsOnDomain::from_vec_and_domain(expanded_domain.fft(&row_poly), expanded_domain);
    let col_evals_on_B =
        EvaluationsOnDomain::from_vec_and_domain(expanded_domain.fft(&col_poly), expanded_domain);
    let val_evals_on_B =
        EvaluationsOnDomain::from_vec_and_domain(expanded_domain.fft(&val_poly), expanded_domain);
...
impl<F: Field> ProverConstraintSystem<F> {
    ...
    /// system
    pub(crate) fn format_random_shares_input(rand_share_input: Vec<F>) -> Vec<F> {
        let mut coeff_0 = F::zero();
        let mut coeff_1 = F::zero();
        let mut i = 0;
        while i < rand_share_input.len() {
            coeff_0 += rand_share_input[i];
            coeff_1 += rand_share_input[i + 1];
            i = i + 2;
        }
        vec![coeff_0, coeff_1]
    }

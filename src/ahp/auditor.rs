...
impl<F: PrimeField> AHPForR1CS<F> {
    ...
    pub fn verifier_query_set<'a, 'b, R: RngCore, C: ConstraintSynthesizer<F>>(
        ...
        let third_round_msg = third_round_msg.expect(
            "VerifierState should include third_round_msg when verifier_query_set is called",
        );
        ...
        let beta_3 =
            beta_3.expect("VerifierState should include beta_3 when verifier_query_set is called");
    ...
    pub fn verifier_decision<C: ConstraintSynthesizer<F>>(
        ...
        let third_round_msg = third_round_msg.expect(
            "VerifierState should include third_round_msg when verifier_decision is called",
        );
        ...
        let beta_3 =
            beta_3.expect("VerifierState should include beta_3 when verifier_decision is called");
        ...
        let a_row_at_beta_3 = *evals
            .get(&("a_row", beta_3))
            .ok_or(Error::MissingEval("a_row"))?;
        let a_col_at_beta_3 = *evals
            .get(&("a_col", beta_3))
            .ok_or(Error::MissingEval("a_col"))?;
        let a_val_at_beta_3 = *evals
            .get(&("a_val", beta_3))
            .ok_or(Error::MissingEval("a_val"))?;
        let b_row_at_beta_3 = *evals
            .get(&("b_row", beta_3))
            .ok_or(Error::MissingEval("b_row"))?;
        let b_col_at_beta_3 = *evals
            .get(&("b_col", beta_3))
            .ok_or(Error::MissingEval("b_col"))?;
        let b_val_at_beta_3 = *evals
            .get(&("b_val", beta_3))
            .ok_or(Error::MissingEval("b_val"))?;
        let c_row_at_beta_3 = *evals
            .get(&("c_row", beta_3))
            .ok_or(Error::MissingEval("c_row"))?;
        let c_col_at_beta_3 = *evals
            .get(&("c_col", beta_3))
            .ok_or(Error::MissingEval("c_col"))?;
        let c_val_at_beta_3 = *evals
            .get(&("c_val", beta_3))
            .ok_or(Error::MissingEval("c_val"))?;
        ...
        let h_3_at_beta_3 = *evals
            .get(&("h_3", beta_3))
            .ok_or(Error::MissingEval("h_3"))?;
        let g_3_at_beta_3 = *evals
            .get(&("g_3", beta_3))
            .ok_or(Error::MissingEval("g_3"))?;
        ...
        let a_at_beta_3 = eta_a
            * &v_H_at_beta_2
            * &v_H_at_beta_1
            * &a_val_at_beta_3
            * &(beta_2 - &b_row_at_beta_3)
            * &(beta_1 - &b_col_at_beta_3)
            * &(beta_2 - &c_row_at_beta_3)
            * &(beta_1 - &c_col_at_beta_3)
            + &(eta_b
                * &v_H_at_beta_2
                * &v_H_at_beta_1
                * &b_val_at_beta_3
                * &(beta_2 - &a_row_at_beta_3)
                * &(beta_1 - &a_col_at_beta_3)
                * &(beta_2 - &c_row_at_beta_3)
                * &(beta_1 - &c_col_at_beta_3))
            + &(eta_c
                * &v_H_at_beta_2
                ...
                * &(beta_2 - &b_row_at_beta_3)
                * &(beta_1 - &b_col_at_beta_3));
        let b_at_beta_3 = (beta_2 - &a_row_at_beta_3)
            * &(beta_1 - &a_col_at_beta_3)
            * &(beta_2 - &b_row_at_beta_3)
            * &(beta_1 - &b_col_at_beta_3)
            * &(beta_2 - &c_row_at_beta_3)
        ...
        let g_2_at_beta_2 = *evals
            .get(&("g_2", beta_2))
            .ok_or(Error::MissingEval("g_2"))?;
        let h_2_at_beta_2 = *evals
            .get(&("h_2", beta_2))
            .ok_or(Error::MissingEval("h_2"))?;

        let lhs = r_alpha_at_beta_2 * &sigma_3;
        let rhs =
            h_2_at_beta_2 * &v_H_at_beta_2 + &(beta_2 * &g_2_at_beta_2 + &(sigma_2 / &h_size));
        ...
        let mask_poly_at_beta_1 = *evals
            .get(&("mask_poly", beta_1))
            .ok_or(Error::MissingEval("mask_poly"))?;
        let r_alpha_at_beta_1 = domain_h.eval_unnormalized_bivariate_lagrange_poly(alpha, beta_1);

        // The additional 1 required by the constraint system.
        //TOO: Make this better
        let x_domain = EvaluationDomain::new(statement_comms_len)
            .ok_or(SynthesisError::PolynomialDegreeTooLarge)
            .unwrap();

        let z_a_at_beta_1 = *evals
            .get(&("z_a", beta_1))
            .ok_or(Error::MissingEval("z_a"))?;
        let z_b_at_beta_1 = *evals
            .get(&("z_b", beta_1))
            .ok_or(Error::MissingEval("z_b"))?;
        let z_c_at_beta_1 = z_a_at_beta_1 * &z_b_at_beta_1;
        let sum_of_z_m =
            eta_a * &z_a_at_beta_1 + &(eta_b * &z_b_at_beta_1) + &(eta_c * &z_c_at_beta_1);

        let x_poly_at_beta_1 = *evals.get(&("x", beta_1)).ok_or(Error::MissingEval("x"))?;
        let w_at_beta_1 = *evals.get(&("w", beta_1)).ok_or(Error::MissingEval("w"))?;
        let z_at_beta_1 =
            w_at_beta_1 * &x_domain.evaluate_vanishing_polynomial(beta_1) + &x_poly_at_beta_1;

        let g_1_at_beta_1 = *evals
            .get(&("g_1", beta_1))
            .ok_or(Error::MissingEval("g_1"))?;
        let h_1_at_beta_1 = *evals
            .get(&("h_1", beta_1))
            .ok_or(Error::MissingEval("h_1"))?;

        let lhs =
            mask_poly_at_beta_1 + &(r_alpha_at_beta_1 * &sum_of_z_m) - &(sigma_2 * &z_at_beta_1);
